...
public class AdminOrderInfoController {
    ...
    void backBtnClck(ActionEvent event) {
        ...
        adminSeatsInfoController.setSelectedSeats(selectedSeats);
        adminSeatsInfoController.setUserNumber(userNumber);
    ...
    void confirmOrderBtnClick(ActionEvent event) {
        ...
        if (result.get() == ButtonType.OK) {
            try {
                dbHandler.addOrder(session.getSessionId(), userNumber, session.getHallId(), selectedSeats);
                Parent root = null;
                try {
    ...
    void initialize() {
        ...
        anchorPane.sceneProperty().addListener(((observableValue, oldValue, newValue) -> {
            if (newValue != null) {
                numberLabel.setText("Номер пользователя: " + userNumber);
